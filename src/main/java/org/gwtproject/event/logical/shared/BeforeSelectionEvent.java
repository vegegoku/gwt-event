...
public class BeforeSelectionEvent<T> extends
   ...
   */
  public static <T> org.gwtproject.event.logical.shared.BeforeSelectionEvent<T> fire(
          HasBeforeSelectionHandlers<T> source, T item) {
    // If no handlers exist, then type can be null.
    if (TYPE != null) {
      org.gwtproject.event.logical.shared.BeforeSelectionEvent<T> event = new org.gwtproject.event.logical.shared.BeforeSelectionEvent<T>();
      event.setItem(item);
